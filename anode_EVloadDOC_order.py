...
class EVChargingOptimizer:
    ...
    def optimizeCommunityChargingPattern(self, community_vehicles_distribution, community_arriving_vehicles,
        ...
        for t in range(self.N_SLOTS):
            constraints.append(model.sum(charge[t]) <= community_vehicles_distribution[t])
        ...
        for t in range(self.N_SLOTS):
            ...
            net_charging_provided = model.sum(
                (charge[t_prime]) * self.P_slow * self.DELTA_T
        ...
        constraints.append(total_charging_provided + total_slcak == total_charging_demand)
        # # v2g 充电-放电 = 0
        ...
        if solution:
            ...
            for t in range(self.N_SLOTS):
                ...
                # 计算EV车主的收益的负荷
                ev_power = solution.get_value(charge[t]) * self.P_slow
                net_power = charging_power
                net_power_per_half_hour.append(net_power)
                ev_vector.append(ev_power)

                # 同时计算这个时间段的P_total
                P_total_this_slot = community_P_BASIC[t] + charging_power
    ...
    def optimizeOfficeChargingPattern(self, slow_vehicles_distribution, slow_arriving_vehicles, slow_leaving_vehicles,
        ...
        for t in range(self.N_SLOTS):
            ...
            constraints.append(model.sum(slow_charge[t])
        ...
        for t in range(self.N_SLOTS):
            ...
            net_charging_provided = model.sum(
                (slow_charge[t_prime]) * self.P_slow * self.DELTA_T
                for t_prime in range(t + 1))
        ...
        if solution:
            ...
            for t in range(self.N_SLOTS):
                ...
                ev_power = solution.get_value(slow_charge[t]) * self.P_slow
                net_power_per_half_hour.append(net_power)
                ev_vector.append(ev_power)

                # 同时计算这个时间段的P_total
                P_total_this_slot = (office_P_BASIC[t] + charging_power +
                                     fast_vehicles_distribution[t] * self.P_quick / self.efficiency)
                P_total_calculated.append(P_total_this_slot)
                total_slcak_list.append(slack_power)
...
def EVload_oreder(EV_Q1, EV_S1, EV_2, EV_3, EV_4, EV_penetration, v2g_ratio, file_path,nodedata_dict, re_capacity_dict):
...
for ev_p in EV_penetration_values:
    for v2g in v2g_ratio_values:
        ...
        node_EV_load, mic_EV_load, node_P_total, node_P_basic_and_EV,mic_EV_load_quick, node_slack_load, P_basic_dict \
            = EVload_oreder(EV_Q1, EV_S1, EV_2, EV_3, EV_4, ev_p, v2g, 'data2')
        ...
        for node in [102, 206, 317, 401]:
            ...
            # Save the figure
            plt.savefig(f'{output_directory}/ev_p_{ev_p}_order_node_{node}.png')
