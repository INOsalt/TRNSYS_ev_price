...
# 老化数据储存
aging_path = "vehicle_cycles/onlyPV_order"
# EV曲线储存
ev_power_path = 'annual_EV_onlyPV_order/'
...
class Node_annual:
    ...
    def year_sim(self):
        ...
        for day in range(365):
            ...
            discharge_row = df_charge.iloc[day]  # 直接读取第day行
        ...
        # 将 error_day 转换为 DataFrame
        df_error_day = pd.DataFrame(error_day, columns=["node_num", "day"])  # 设置列名
        # 保存到 CSV 文件
        file_name_error = os.path.join(aging_folder_path, f"{self.node_num}_error.csv")
        df_error_day.to_csv(file_name_error, index=False)  # 不写入索引
